...
elements = { 1:'H', 2:'He', 3:'Li', 4: 'Be', 5: 'B', 6:'C', 7:'N', 8:'O', 9:'F', 10: 'Ne', 11: 'Na', 12: 'Mg', 13:'Al', 14:'Si', 15:'P', 
...
gap_file = sys.argv[1]
file = open(gap_file, 'rt'); f = file.read()
print("Reading " + sys.argv[1] + "...")

if len(sys.argv) > 3:
	hirs = sys.argv[3]
	file_h = open(hirs, 'rt'); f_h = file_h.read()
...
p = Path(output)
if p.is_file():
        print("WARNING: you have already output file in the folder with the same name. Remove the file and execute script again")
        exit()
	...
	if n_mb == 0:
		...
		with open(output, 'a') as f3:
			...
			if ns == []:
				...
				spec_tmp4 = [int(x) for x in str(spec_tmp3)]
				spec_tmp5 = []
				for n in spec_tmp4:
					tmp=elem(n)
