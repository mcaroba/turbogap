...
LO, HI = 2193, 2560          # inclusive, 1-based; 2560 is the closing #endif

ARGS = """params rank ierr n_sites n_species md_istep md_time time_step
positions positions_prev positions_diff velocities forces forces_prev masses masses_types
...
energies_vdw energies_lp energies_exp energies_pdf energies_sf energies_xrd energies_nd
local_properties local_property_labels
...
filename string allelstopdata ephbeta ephfdm ephlsc
time_md time_mpi_positions
cum_eel gd_box_do_pos gd_istep restart_box_optim target_temp time_step_prev""".split()

# filename and string are used as internal files (write (filename, ...)), which
# the standard forbids for intent(in), so they stay inout.
IN = set("""allelstopdata e_kin energies energy energy_prev evpera3tobar fix_atom
i_image i_nested instant_pressure_prev kb local_properties local_property_labels masses
masses_types nrows rank v_uc virial xyz xyz_species""".split())
