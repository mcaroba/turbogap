...
BUCKETS = {
    ...
    # GPU branch, off-pattern names for the same kind of bucket.
    "soap_time_soap": "soap_solo",
    "solo_time_soap": "soap_lin",
}

# Names that match the time_* shape and are NOT wall-clock buckets.
NOT_TIMERS = ("time_step", "time_step_prev")

# Bucket names that do not start with "time_", so the leftover check
# below cannot find them by prefix.
ODD_NAMES = ("soap_time_soap", "solo_time_soap")
...
def main(argv):
    ...
    for path in files:
        ...
        # Set difference binds tighter than union: without the parentheses the
        # subtractions apply to the second operand only, and time_step comes
        # back as a false positive.
        found = set(re.findall(r"\btime_[a-z0-9_]+\b", out)) | (
            set(re.findall(r"\b[a-z_]+_time_[a-z0-9_]+\b", out)) & set(ODD_NAMES)
        )
        leftover = sorted(found - set(NOT_TIMERS) - {"time_start", "time_end"})
