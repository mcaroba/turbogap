...
clean:
	rm -rf $(OBJ_CUDA) $(OBJ_CC) $(OBJ_TP_BT) $(OBJ_ST) $(OBJ_STOP) $(OBJ) $(INC_DIR)/*.mod $(PROG)
...
$(BUILD_DIR)/%.o: src/%.cc
	$(CC) $(CC_OPTS) -c $< -o $@
$(BIN_DIR)/%: src/%.f90 $(OBJ_TP_BT) $(OBJ_ST) $(OBJ_STOP) $(OBJ) $(OBJ_CUDA) $(OBJ_CC) | $$(@D)
	$(F90) $(PP) $(F90_OPTS) $< -o $@ $(OBJ_TP_BT) $(OBJ_ST) $(OBJ_STOP) $(OBJ) $(OBJ_CUDA) $(OBJ_CC) $(LIBS)
