...
! These is the type for the input parameters
  type input_parameters
    ...
    real*8 :: t_beg = 300.d0, t_end = 300.d0, tau_t = 100.d0, md_step = 1.d0, &
              ...
              gamma0 = 0.01d0, max_opt_step = 0.1d0, t_extra = 0.d0, p_nested = 0.d0, nested_max_strain = 0.d0, &
              nested_max_volume_change = 0.d0
    integer :: md_nsteps = 1, write_xyz = 0, write_thermo = 1, which_atom = 0, n_nested = 0
    ...
    logical :: do_md = .false., do_prediction = .false., do_forces = .false., do_derivatives = .false., &
               ...
               variable_time_step = .false., do_nested_sampling = .false., scale_box_nested = .false.
...
! This is a container for atomic images
  type image
    real*8, allocatable :: positions(:,:), positions_prev(:,:), velocities(:,:), masses(:), &
                           forces(:,:), forces_prev(:,:)
    real*8 :: a_box(1:3), b_box(1:3), c_box(1:3), energy
...
!   Input variables
    real*8, intent(in) :: positions(:,:), velocities(:,:), masses(:), &
                           forces(:,:), a_box(1:3), b_box(1:3), c_box(1:3), energy
...
!   Output variables
    real*8, allocatable, intent(out) :: positions(:,:), velocities(:,:), masses(:), &
                           forces(:,:)
    real*8, intent(out) :: a_box(1:3), b_box(1:3), c_box(1:3), energy
