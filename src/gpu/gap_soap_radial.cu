...
TG_INLINE_FUNCTION double N_a(double rcut, int a) {
  const int b = 2 * a + 5;
  return sqrt(rcut / static_cast<double>(b));
}


static void cuda_global_scaling(double* radial_exp_coeff_d, int* i_beg_d, int* i_end_d, double* global_scaling_d, int n_max,
                                int n_atom_pairs, int n_species, double* rcut_hard_d, int* k2_i_site_d, int* k2_start_d, int divide,
                                hipStream_t* stream) {
  tg_parallel_for(
      "turbogap_soap_global_scaling", n_atom_pairs, stream,
      TG_LAMBDA(const int i_ij) {
...
extern "C" void gpu_get_radial_exp_coeff_poly3gauss(double* radial_exp_coeff_d, double* radial_exp_coeff_der_d, int* i_beg_d,
  ...
  int divide;
  divide = 0;
  cuda_global_scaling(radial_exp_coeff_d, i_beg_d, i_end_d, global_scaling_d, size_radial_exp_coeff_one, size_radial_exp_coeff_two,
                      n_species, rcut_hard_d, k2_i_site_d, k_2start_d, divide, stream);
  /* gpuErrchk( hipPeekAtLastError() );
       gpuErrchk( hipDeviceSynchronize() ); */
  if (c_do_derivatives) {
    divide = 1;
    cuda_global_scaling(radial_exp_coeff_der_d, i_beg_d, i_end_d, global_scaling_d, size_radial_exp_coeff_one,
                        size_radial_exp_coeff_two, n_species, rcut_hard_d, k2_i_site_d, k_2start_d, divide, stream);
  }
  /* gpuErrchk( hipPeekAtLastError() );
       gpuErrchk( hipDeviceSynchronize() );   */
}

static void kernel_get_radial_poly3gauss(int n_atom_pairs, int n_species, bool* mask_d, double* rjs_d, double* rcut_hard_d,
                                         ...
                                         hipStream_t* stream) {
  tg_parallel_for(
      "turbogap_soap_radial_poly3gauss", n_atom_pairs, stream,
            ...
            if (rjs <= rcut_hard_in) {
              ...
              if (k_ij > j) {
                ...
                for (n = -1; n <= alpha_max_der - 1; n++) {
                  ...
                  I_np2 =
                      s2 * double(n + 1) * N_n / N_np2 * I_n - N_np1 * (rj - rcut_hard) / N_np2 * I_np1 + C1 / N_np2 - C2 / N_np2;
                  if (n > 0)
                ...
                if (false || (rcut_soft - rj) < 4.0 * atom_sigma_scaled) {
                  ...
                  for (n = -1; n <= alpha_max_der - 1; n++) {
                    ...
                    double I_np2 =
                        sf2 * double(n + 1) * N_n / N_np2 * I_n - N_np1 * (rj_f - rcut_hard) / N_np2 * I_np1 - C2 / N_np2;
                    if (n > 0)
                  ...
                  if (do_derivatives) {
                    ...
                    double der_pref_f =
                        pref_f * ((rcut_soft - rj) / denom +
                                  ((rcut_soft - rj) * (rcut_soft - rj)) / (denom * denom) * atom_sigma_scaled * atom_sigma_scaling);
                    double der_rjf_rj =
                        (2.0 * atom_sigma_scaled * rcut_soft * atom_sigma_scaling + tmp1) / denom -
                ...
                if (false || rj < 4.0 * (atom_sigma + atom_sigma_scaled)) {
                  ...
                  exp_coeff_temp1_d[k_ij * n_temp + alpha_max - 1] =
                      exp(-0.5 * (rj * rj) / (sigma_star * sigma_star)) * sqrt(pi / 2.0) * atom_sigma_scaled * atom_sigma /
                      sigma_star * (1.0 + erf(atom_sigma / atom_sigma_scaled * rj / sq2 / sigma_star)) * N_gauss;
                  if (do_derivatives)
                    exp_coeff_der_temp_d[k_ij * n_temp_der + alpha_max - 1] =
                        ((rj * rj) * atom_sigma_scaling / (atom_sigma_scaled * atom_sigma_scaled * atom_sigma_scaled) -
                         ...
                         2.0 * (rj * rj) * atom_sigma_scaling * (atom_sigma * atom_sigma) /
                            ...
                            exp_coeff_temp1_d[k_ij * n_temp + alpha_max - 1] +
                        (1. / s2 - 2.0 * rj * atom_sigma_scaling / (atom_sigma_scaled * atom_sigma_scaled * atom_sigma_scaled)) *
                            s2 * (atom_sigma * atom_sigma) / (sigma_star * sigma_star) * sqrt(2.0 / atom_sigma) / sqrt(sqrt(pi)) *
...
extern "C" void gpu_radial_poly3gauss(int n_atom_pairs, int n_species, bool* mask_d, double* rjs_d, double* rcut_hard_d,
  ...
  kernel_get_radial_poly3gauss(n_atom_pairs, n_species, mask_d, rjs_d, rcut_hard_d, n_sites, n_neigh_d, n_max, n_temp,
                               do_derivatives, exp_coeff_d, exp_coeff_der_d, rcut_soft_d, atom_sigma_d, exp_coeff_temp1_d,
                               exp_coeff_temp2_d, exp_coeff_der_temp_d, i_beg, i_end, atom_sigma_scaling_d, mode,
                               radial_enhancement, amplitude_scaling_d, alpha_max_d, nf_d, n_temp_der, W_d, stream);
}

static void kernel_get_radial_poly3(int n_atom_pairs, int n_species, bool* mask_d, double* rjs_d, double* rcut_hard_d, int n_sites,
  ...
  tg_parallel_for(
      "turbogap_soap_radial_poly3", n_atom_pairs, stream,
      TG_LAMBDA(const int k_ij) {
        int n, d, k;
        double ampli_tude, ampli_tude_der, atom_sigma_scaled, amplitude_scaling, C1, C2, W_exp, nf, atom_sigma_f, rj_f, sf2;
        double tmp1, tmp2, tmp3, tmp4, tmp5, tmp6, tmp7;
...
extern "C" void gpu_radial_poly3(int n_atom_pairs, int n_species, bool* mask_d, double* rjs_d, double* rcut_hard_d, int n_sites,
  ...
  kernel_get_radial_poly3(n_atom_pairs, n_species, mask_d, rjs_d, rcut_hard_d, n_sites, n_neigh_d, n_max, n_temp, do_derivatives,
                          exp_coeff_d, exp_coeff_der_d, rcut_soft_d, atom_sigma_d, exp_coeff_temp1_d, exp_coeff_temp2_d,
                          exp_coeff_der_temp_d, i_beg, i_end, atom_sigma_scaling_d, mode, radial_enhancement, amplitude_scaling_d,
                          alpha_max_d, nf_d, n_temp_der, W_d, do_central_d, central_weight_d, stream);
}
