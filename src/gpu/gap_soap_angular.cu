...
// The three indices are the site, the radial n and the angular k. Each
// triple runs a serial neighbour loop and writes one cnk entry, reading
// nothing another triple writes, so the order they are visited in does not
// matter -- which is what makes an MDRange safe here. See tg_parallel_for_3d.
static void cuda_get_cnk_one_new_new(hipDoubleComplex* cnk_d, double* radial_exp_coeff_d, hipDoubleComplex* angular_exp_coeff_d,
                                     int* n_neigh_d, int* k2_start_d, int n_atom_pairs, int n_sites, int k_max, int n_max,
                                     int l_max, hipStream_t* stream) {
  tg_parallel_for_3d(
      "turbogap_soap_cnk", n_sites, n_max, k_max, stream,
      TG_LAMBDA(const int i_site, const int i_n, const int i_k) {
        const int n = i_n + 1;
        const int k = i_k + 1;
...
static void cuda_get_cnk_two(hipDoubleComplex* cnk_d, double* atom_sigma_r, double* atom_sigma_t, double* rcut_hard,
                             double* central_weight, int* species, int* i_beg, int* i_end, int radial_enhancement,
                             int* species_multiplicity, double* W, double* S, int n_sites, int k_max, int n_max, int size_species_1,
                             hipStream_t* stream) {
  tg_parallel_for(
      "turbogap_soap_cnk_two", n_sites, stream,
...
extern "C" void gpu_get_cnk(double* radial_exp_coeff_d, hipDoubleComplex* angular_exp_coeff_d, hipDoubleComplex* cnk_d,
  ...
  /*hipEventRecord(start);
      ...
      for(int lll=1;lll<=1000;lll++){*/
  cuda_get_cnk_one_new_new(cnk_d, radial_exp_coeff_d, angular_exp_coeff_d, n_neigh_d, k2_start_d, n_atom_pairs, n_sites, k_max,
                           n_max, l_max, stream);
  /*}
      hipEventRecord(stop);
      hipEventSynchronize(stop);
  ...
  if (bintybint == 1000) {
    ...
    cuda_get_cnk_two(cnk_d, atom_sigma_r_d, atom_sigma_t_d, rcut_hard_d, central_weight_d, species_d, i_beg_d, i_end_d,
                     radial_enhancement, species_multiplicity_d, W_d, S_d, n_sites, k_max, n_max, size_species_1, stream);
  }
}

static void cuda_get_plm_arrays_one(double* plm_array_global_d, int kmax, int lmax, double* thetas_d, int n_atom_pairs,
                                    hipStream_t* stream) {
  tg_parallel_for(
      "turbogap_soap_plm_arrays_one", n_atom_pairs, stream,
      TG_LAMBDA(const int k_ij) {
...
extern "C" void gpu_get_plm_array_global(double* plm_array_global_d, int n_atom_pairs, int kmax, int lmax, double* thetas_d,
  ...
  dim3 nthreads = dim3(tpb, 1, 1);
  cuda_get_plm_arrays_one(plm_array_global_d, kmax, lmax, thetas_d, n_atom_pairs, stream);
}


static void cuda_get_exp_coeff_one(hipDoubleComplex* eimphi_global_d, double* rjs_d, double* phis_d, bool* mask_d,
                                   ...
                                   double* preflm_d, double* plm_array_global_d, hipDoubleComplex* exp_coeff_d,
                                   hipStream_t* stream) {
  tg_parallel_for(
      "turbogap_soap_exp_coeff_one", n_atom_pairs, stream,
      TG_LAMBDA(const int k_ij) {
        double xcut = 1.0e-7;
...
static void cuda_get_exp_coeff_der_one(hipDoubleComplex* eimphi_global_d, double* rjs_d, double* phis_d, bool* mask_d,
                                       double* atom_sigma_in_d, double* atom_sigma_scaling_d, double rcut, int n_atom_pairs,
                                       int n_species, int lmax, int kmax, double* prefl_global_d, hipDoubleComplex* prefm_global_d,
                                       double* preflm_d, double* prefl_global_der_d, double* plm_array_global_d,
                                       double* plm_array_global_der_d, hipDoubleComplex* exp_coeff_d,
  ...
  tg_parallel_for(
      "turbogap_soap_exp_coeff_der_one", n_atom_pairs, stream,
      TG_LAMBDA(const int k_ij) {
        double rj = rjs_d[k_ij];
        /*double phi=phis_d[k_ij];*/
        if (rj < rcut) {
...
static void cuda_get_plm_arrays_der_one(double* plm_array_global_der_d, int kmax, int lmax, double* thetas_d, int n_atom_pairs,
                                        double* plm_array_div_sin, double* plm_array_der_mul_sin, hipStream_t* stream) {
  tg_parallel_for(
      "turbogap_soap_plm_arrays_der_one", n_atom_pairs, stream,
      TG_LAMBDA(const int k_ij) {
        //double x=cos(thetas_d[k_ij]);
        double part1, part2;
        for (int l = 0; l <= lmax; l++) {
...
extern "C" void gpu_get_exp_coeff_array(hipDoubleComplex* eimphi_global_d, double* rjs_d, double* phis_d, double* thetas_d,
  ...
  cuda_get_exp_coeff_one(eimphi_global_d, rjs_d, phis_d, mask_d, atom_sigma_in_d, atom_sigma_scaling_d, rcut, n_atom_pairs,
                         n_species, lmax, kmax, prefl_global_d, prefm_global_d, preflm_d, plm_array_global_d, exp_coeff_d,
                         stream); //, fact_array_d);


  if (c_do_derivatives) {
    cuda_get_plm_arrays_der_one(plm_array_global_der_d, kmax, lmax, thetas_d, n_atom_pairs, plm_array_div_sin,
                                plm_array_der_mul_sin, stream);


    cuda_get_exp_coeff_der_one(eimphi_global_d, rjs_d, phis_d, mask_d, atom_sigma_in_d, atom_sigma_scaling_d, rcut, n_atom_pairs,
                               n_species, lmax, kmax, prefl_global_d, prefm_global_d, preflm_d, prefl_global_der_d,
                               plm_array_global_d, plm_array_global_der_d, exp_coeff_d, eimphi_rad_der_global_d,
                               eimphi_azi_der_global_d, plm_array_div_sin, plm_array_der_mul_sin, exp_coeff_rad_der_d,
                               exp_coeff_azi_der_d, exp_coeff_pol_der_d, stream);
  }
