...
#include <limits>
template <bool is_gauss> __global__ void init_S(int alpha_max, double* S) {
...
void sqrt_mat(double* W, const int alpha_max, hipStream_t& s, hipblasHandle_t& handle) {
  ...
  {
    auto status = hipsolverDnDgesvd(handle, 'A', 'A', alpha_max, alpha_max, W, alpha_max, svd, U, alpha_max, VT, alpha_max, work,
                                    lworksize, nullptr, devinfo);
    if (status != HIPSOLVER_STATUS_SUCCESS) {
      printf("\n\n ERROR, status is: %d \n\n", status);
      throw std::runtime_error("");
    }
  ...
  hipMalloc((void**) &tmpmat, alpha_max * alpha_max * sizeof(double));
  {
    ...
    auto alpha = 1.0;
    auto beta = 0.0;
    auto status = hipblasDgemm(handle, HIPBLAS_OP_N, HIPBLAS_OP_N, alpha_max, alpha_max, alpha_max, &alpha, U, alpha_max, W,
                               alpha_max, &beta, tmpmat, alpha_max);
  ...
  }
  {
    ...
    auto alpha = 1.0;
    auto beta = 0.0;
    auto status = hipblasDgemm(handle, HIPBLAS_OP_N, HIPBLAS_OP_N, alpha_max, alpha_max, alpha_max, &alpha, tmpmat, alpha_max, VT,
                               alpha_max, &beta, W, alpha_max);
    if (status != HIPBLAS_STATUS_SUCCESS) {
      std::cout << "ERROR IN hipblasDgemm, status is" << status << std::endl;
      throw std::runtime_error("");
    }
...
void invert_mat(double*& W, const int alpha_max, hipStream_t& s, hipblasHandle_t& handle) {
  ...
  {
    auto status = hipsolverDgetrs_bufferSize(handle, HIPSOLVER_OP_N, alpha_max, alpha_max, W, alpha_max, Ipivot, IdMat, alpha_max,
                                             &lworksize);
    if (status != HIPSOLVER_STATUS_SUCCESS) {
      printf("\n\n ERROR, status is: %d \n\n", status);
      throw std::runtime_error("");
  ...
  }
  {
    ...
    auto status = hipsolverDgetrs(handle, HIPSOLVER_OP_N, alpha_max, alpha_max, W, alpha_max, Ipivot, IdMat, alpha_max, work,
                                  lworksize, devinfo);
    if (status != HIPSOLVER_STATUS_SUCCESS) {
      printf("\n\n ERROR, status is: %d \n\n", status);
      throw std::runtime_error("");
...
void get_orthonormalization_matrix_poly3gauss_cc(const int alpha_max, const double atom_sigma_in, const double rcut_hard_in,
                                                 double* S, double* W, hipblasHandle_t handle, hipStream_t s) {
  //local data(?)
  double* edge_S;
  double* edge_SH;
...
void orthonormalization_copy_to_global_matrix_cc(double* src, double* dest, const int src_rowsize, const int dest_start,
                                                 const int dest_rowsize, hipStream_t s) {
...
void get_orthonormalization_matrix_poly3gauss(const int alpha_max, const double atom_sigma_in, const double rcut_hard_in, double* S,
                                              double* W, hipblasHandle_t* handle, hipStream_t* s) {
...
void copy_to_global_matrix(double* src, double* dest, const int src_rowsize, const int dest_start, const int dest_rowsize,
                           hipStream_t* s) {
