...
!   MBD stuff:
    real*8, allocatable :: T_LR(:,:), r0_ii_SCS(:), f_damp_SCS(:), AT(:,:,:), AT_n(:,:,:,:), energy_series(:,:), &
                           ...
                           dr6_mult_0i(:), dr6_mult_0j(:), a_iso_der(:,:,:)
    real*8 :: a_mbd_i, a_mbd_j, da_i, da_j, pol1, E_TS, f_damp_der_2b, dr_vdw_i, &
              dr_vdw_j, forces_TS, dC6_2b, mult1_i, mult1_j, mult2, dmult1_i(1:3), dmult1_j(1:3), dmult2(1:3), hv_p_der, &
...
!   Number of frequencies
    ...
    do i = 1, n_sites      
      ...
      if ( do_derivatives ) then
        ...
        allocate( a_iso_der(1:n_neigh(i),1:3,1:2) )
        da_iso = 0.d0
        a_iso_der = 0.d0
      end if
      if ( do_derivatives .and. do_hirshfeld_gradients ) then
      ...
      do om = 1, 2
        ...
        if ( polynomial_expansion ) then
          ...
          !call dsysv( 'U', 3*n_sub_sites, 3, B_mat, 3*n_sub_sites, ipiv, a_SCS, 3*n_sub_sites, work_arr, &
          !            12*n_sub_sites, info )
          call dsytrf( 'U', 3*n_sub_sites, B_mat, 3*n_sub_sites, ipiv, work_arr, &
                      12*n_sub_sites, info )
          call dsytri( 'U', 3*n_sub_sites, B_mat, 3*n_sub_sites, ipiv, work_arr, &
                      12*n_sub_sites, info )
          call dsymm( 'L', 'U', 3*n_sub_sites, 3, 1.d0, B_mat, 3*n_sub_sites, d_vec, &
        ...
        if (do_derivatives) then
          ...
          do c3 = 1, 3
            ...
            if ( polynomial_expansion ) then
              ...
              !            3*n_sub_sites, 0.d0, a_SCS, 3*n_sub_sites )
              do c1 = 1, 3
                a_iso_der(a2,c3,om) = a_iso_der(a2,c3,om) + dot_product( B_mat(c1,:), b_der(:,c1) )
              end do

              a_iso_der(a2,c3,om) = 1.d0/3.d0 * a_iso_der(a2,c3,om)
              if ( c3 == 1 .and. om == 2 ) then
                write(*,*) "a_iso_der", i, a, a_iso_der(a2,c3,om)
              end if            
