...
program turbogap
  ...
  real*8, allocatable :: energies(:), forces(:,:), energies_soap(:),&
       ...
       & masses_temp(:), sinc_factor_matrix(:,:)
...
!  real*8, pointer :: this_hirshfeld_v_pt(:), this_hirshfeld_v_cart_der_pt(:,:)
  ...
  real*8 :: instant_temp, kB = 8.6173303d-5, E_kinetic=0.d0, E_kinetic_prev, time1, time2, time3, time_neigh, &
       ...
       time_mpi(1:3) = 0.d0, time_core_pot(1:3), time_vdw(1:3), time_pdf(1:3), time_sf(1:3), instant_pressure, lv(1:3,1:3), &
  ...
  real*8, allocatable :: temp_1d(:), temp_1d_bis(:), temp_2d(:,:),&
       ...
       & y_structure_factor(:), y_structure_factor_temp(:), x_xrd(:),&
       & x_xrd_temp(:), y_xrd(:), y_xrd_temp(:), y_xrd_der(:,:,:), y_xrd_temp_der(:,:,:)
...
!     > experimental_forces = .true. will add forces to the calculation
        ...
        if( any( soap_turbo_hypers(:)%has_core_electron_be ) .and.( params%do_prediction ) &
           ...
           call get_exp_pred_spectra_energies_forces( params%exp_energy_scales(xps_idx),&
        ...
        if (params%do_pair_distribution)then
           ...
           ! call get_pdf_sf_xrd_explicitly_kde( v_uc, n_sites, n_species, species, params%species_types,  &
           !      & neighbors_list, n_neigh, neighbor_species, rjs, xyz, params%pair_distribution_rcut, &
           !      & params%r_range_min, params%r_range_max, params&
           !      &%pair_distribution_n_samples,&
           !      & pair_distribution_partial,&
           !      & pair_distribution_partial_der, params&
           !      &%pair_distribution_kde_sigma,  params%exp_forces,&
           !      & rank )
           ...
           if (rank == 0) print *, rank, " TIMEPDF = ", time_pdf(3)
        ...
        if (params%do_structure_factor )then
           ...
           if (rank == 0) print *, rank, " TIMESF = ", time_sf(3)
        ...
        if( params%do_prediction )then
           ...
           if( allocated(this_energies_pdf) )then
              counter2 = counter2 + 1
           end if
           if( allocated(this_energies_sf) )then
              counter2 = counter2 + 1
           end if
           if( allocated(this_energies_xrd) )then
              counter2 = counter2 + 1
           end if
