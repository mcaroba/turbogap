...
!   These are the routines for calculating the monte-carlo acceptance criteria for different move types
   ...
   subroutine get_mc_acceptance(mc_move, p_accept, energy, energy_prev, temp, &
      ...
      real(dp), allocatable :: mass_in(:), mu_in(:)
...
!          Now pick a random index and displace
   ...
   subroutine get_mc_move(n_sites, n_mc_mu, n_mc_species, mc_types, mc_move, acceptance, mu_acceptance,&
      ...
      do while (invalid_move)
         ...
         if (allocated(n_mc_species) .and. mc_move == "removal") then
   ...
   subroutine mc_insert_site(mc_species, mc_id, positions, ref_positions, idx, n_sites, a_box, b_box, c_box, &
                             indices, species, ref_species, xyz_species, ref_xyz_species, min_dist, max_dist, &
      ...
      real(dp), intent(in) :: a_box(1:3), b_box(1:3), c_box(1:3), min_dist, max_dist
      real(dp) :: ranv(1:3)
...
!    real(dp) :: mc_max_dist
      integer :: mc_n_planes
      real(dp), allocatable :: mc_planes(:), mc_max_dist_to_planes(:) ! Final index indexes the planes in first index
      ...
      do while (.true.)
         ...
         if (mc_n_planes > 0) then
            if (mc_planes_restrict_to_polyhedron) then
               call check_in_polyhedron(positions(1:3, n_sites), mc_n_planes, mc_planes, too_far_planes)
            else
   ...
   subroutine check_in_polyhedron(position, n_planes, planes, not_in_polyhedron)
      ...
      real(dp) :: closest_point_on_plane(1:3), normal(1:3), d
      real(dp) :: dist, maximum = 10000000.d0
   ...
   subroutine check_if_far_from_planes(position, n_planes, planes, max_dist_to_planes, too_far_out)
      implicit none
      integer :: i, n_planes
      real(dp), allocatable :: planes(:), max_dist_to_planes(:)
      real(dp), intent(in) :: position(1:3)
      real(dp) :: closest_point_on_plane(1:3), normal(1:3), d
      real(dp) :: dist, maximum = 10000000.d0
      logical :: too_far
   ...
   subroutine check_if_atoms_too_close_or_far(position, ref_positions, n_sites, &
      ...
      real(dp), intent(in) :: position(:), ref_positions(:, :), min_dist, max_dist
      real(dp), intent(in) :: a_box(1:3), b_box(1:3), c_box(1:3)
      real(dp) :: d, dist(1:3), minimum = 10000000.d0
      logical :: too_close
      logical :: too_far
   ...
   subroutine perform_mc_step(&
        ...
        & mc_move_max, mc_min_dist, mc_max_dist, mc_max_insertion_trials, ln_vol_max, mc_types, masses_types,&
      ...
      real(dp), intent(inout) :: disp(1:3), mc_min_dist, mc_max_dist, d_disp,&
      ...
      real(dp) :: gamma(1:6)
      real(dp), allocatable :: mc_planes(:), mc_max_dist_to_planes(:)
