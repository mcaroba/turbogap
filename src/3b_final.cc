...
__global__ void eval_pref(double* __restrict__ pref, const double* __restrict__ alpha, const double delta,
                          const double* __restrict__ cutoff, const int size) {
...
__device__ void eval_energies_d(const double rcut, const double buffer, const int n_sparse, const double* __restrict__ xyz,
                                const double* __restrict__ xyz_red, const double* __restrict__ r, const double* __restrict__ sigma,
                                const double* __restrict__ qs, const double* __restrict__ pref, double* energies, double* forces,
                                double* virial, double* fcut, double* dfcut, const int i, const int i3, const int j3, const int k3,
                                const int n_sites0) {
  double q[3];
  ...
  if (tid < 6) {
    if (r[tid / 3] < rcut - buffer) {
      if (tid % 3 == 0)
        fcut[tid / 3] = 1.0; //need to assign only once, no bidimensional
      dfcut[tid] = 0.0;
    } else {
      if (tid % 3 == 0)
        fcut[tid / 3] = (cos(pi * (r[tid / 3] - rcut + buffer) / buffer) + 1.0) / 2.0; //need to assign only once, no bidimensional
  ...
  if (tid < 3) {
    if (tid % 3 == 0)
      fcut[2] = fcut[0] * fcut[1]; // fcut = fcut 12*fcut 13 has been put in same array.
  }
  __syncthreads();
  if (tid < 3) {
...
__global__ void kernel_2nd_try(
    ...
    const int* neighbor_species, const int* kappas_array, const double rcut, const double buffer,
    //const double e0   ,
    //device array
    const double* sigma, const double* qs, const double* pref_d,
    //output arrays
    double* energies, double* forces, double* virial, const int i_beg) {
  ...
  for (int j = 1; j < n_neigh[i]; ++j) {
    ...
    for (int j2 = j + 1; j2 < n_neigh[i]; ++j2) {
      ...
      if (!(((neighbor_species[k] == sp1 && neighbor_species[k2] == sp2) ||
             (neighbor_species[k] == sp2 && neighbor_species[k2] == sp1)) &&
            r_shmem[1] < rcut)) {
      ...
      if constexpr (type == exp_k) {
        if constexpr (do_forces == true) {
          eval_energies_d<true, exp_k>(rcut, buffer, n_sparse, xyz_shmem, xyz_red_shmem, r_shmem, sigma, qs, pref_d, energies,
                                       forces, virial_loc, fcut, dfcut, i, i3, j3, k3, n_sites0);
        } else {
          eval_energies_d<false, exp_k>(rcut, buffer, n_sparse, xyz_shmem, xyz_red_shmem, r_shmem, sigma, qs, pref_d, energies,
                                        forces, virial_loc, fcut, dfcut, i, i3, j3, k3, n_sites0);
        }
      }
      ...
      if constexpr (type == pol_k) {
        if constexpr (do_forces == true) {
          eval_energies_d<true, pol_k>(rcut, buffer, n_sparse, xyz_shmem, xyz_red_shmem, r_shmem, sigma, qs, pref_d, energies,
                                       forces, virial_loc, fcut, dfcut, i, i3, j3, k3, n_sites0);
        } else {
          eval_energies_d<false, pol_k>(rcut, buffer, n_sparse, xyz_shmem, xyz_red_shmem, r_shmem, sigma, qs, pref_d, energies,
                                        forces, virial_loc, fcut, dfcut, i, i3, j3, k3, n_sites0);
        }
      }
    } //j2 loop
...
//    __global__ void sanity_check(const double* rjs, const int* neighbors_list, const int n_atom_pairs){
__global__ void sanity_check(const int n_sparse, const int n_sites, const int n_atom_pairs, const int n_sites0, const int sp0,
                             const int sp1, const int sp2, const double* alpha, const double delta, const double* cutoff,
                             //host arrays
                             const double* rjs, const double* xyz, const int* n_neigh, const int* species,
                             const int* neighbors_list, const int* neighbor_species, const int* kappas_array, const double rcut,
                             const double buffer,
                             //const double e0   ,
                             //device array
                             const double* sigma, const double* qs, const double* pref_d,
                             //output arrays
                             double* energies, double* forces, double* virial) {
  printf("alpha is:\n");
...
void setup_3bresult_arrays(void** energies, void** forces, void** virial, const hipStream_t s, const bool do_forces,
                           const int n_sites, const int n_sites0) {
...
void cleanup_3bresult_arrays(void** energy_d, void** forces_d, void** virials_d, void** energy_h, void** forces_h, void** virials_h,
                             const hipStream_t s, const bool do_forces, const int n_sites, const int n_sites0) {
  if (do_forces) {
...
void gpu_3b_cc_2nd_try(
    ...
    const int n_sparse, const int n_sites, const int n_atom_pairs, const int n_sites0, const int sp0, const int sp1, const int sp2,
    const double* alpha, const double delta, const double e0, const double* cutoff, const hipStream_t s,
    //host arrays
    const double* rjs, const double* xyz, const int* n_neigh, const int* species, const int* neighbors_list,
    const int* neighbor_species, const bool do_forces, const double rcut, const double buffer,
    //device array
    const double* sigma, const double* qs, const kern_type kernel_type, double* energies, double* forces, double* virial,
    const int i_beg, const int i_end, const int* kappas_array_d) {
  //const auto pi = std::numbers::pi_v<double>;
  const auto pi = 3.14159265358979323846264338327950288419716939; // std::numbers::pi_v<double>;
  double* pref_d;
  double* pref_h;
  ...
  switch (kernel_type) {
  case exp_k: {
    do_forces ? kernel_2nd_try<true, exp_k>
        <<<grids, threads, 0, s>>>(n_sparse, n_sites, n_atom_pairs, n_sites0, sp0, sp1, sp2, alpha, delta, cutoff, rjs, xyz,
                                   n_neigh, species, neighbors_list, neighbor_species, kappas_array_d, rcut, buffer, /*e0,*/ sigma,
                                   qs, pref_d, energies, forces, virial, i_beg)
              : kernel_2nd_try<false, exp_k>
        <<<grids, threads, 0, s>>>(n_sparse, n_sites, n_atom_pairs, n_sites0, sp0, sp1, sp2, alpha, delta, cutoff, rjs, xyz,
                                   n_neigh, species, neighbors_list, neighbor_species, kappas_array_d, rcut, buffer, /*e0,*/ sigma,
                                   qs, pref_d, energies, forces, virial, i_beg);
    break;
  ...
  case pol_k: {
    do_forces ? kernel_2nd_try<true, pol_k>
        <<<grids, threads, 0, s>>>(n_sparse, n_sites, n_atom_pairs, n_sites0, sp0, sp1, sp2, alpha, delta, cutoff, rjs, xyz,
                                   n_neigh, species, neighbors_list, neighbor_species, kappas_array_d, rcut, buffer, /*e0,*/ sigma,
                                   qs, pref_d, energies, forces, virial, i_beg)
              : kernel_2nd_try<false, pol_k>
        <<<grids, threads, 0, s>>>(n_sparse, n_sites, n_atom_pairs, n_sites0, sp0, sp1, sp2, alpha, delta, cutoff, rjs, xyz,
                                   n_neigh, species, neighbors_list, neighbor_species, kappas_array_d, rcut, buffer, /*e0,*/ sigma,
                                   qs, pref_d, energies, forces, virial, i_beg);
    break;
...
void gpu_3b(const int n_sparse, const int n_sites, const int n_atom_pairs, const int n_sites0, const int sp0, const int sp1,
  ...
  gpu_3b_cc_2nd_try(n_sparse, n_sites, n_atom_pairs, n_sites0, sp0, sp1, sp2, alpha, delta, e0, cutoff, *s, rjs, xyz, n_neigh,
                    species, neighbors_list, neighbor_species, do_forces, rcut, buffer, sigma, qs, k, energy, forces, virials,
                    i_beg, i_end, kappas_array_d);
}


void create_kappas_cwrap(int* kappas_array_d, int* n_neigh_host, const hipStream_t* s, const int n_sites) {
  create_kappas(kappas_array_d, n_neigh_host, *s, n_sites);
}

//allocate and 0 init device arrays
void setup_3bresult_arrays_cwrap(void** energy_d, void** forces_d, void** virials_d, const hipStream_t* s, const bool do_forces,
                                 const int n_sites, const int n_sites0) {
...
void cleanup_3bresult_arrays_cwrap(void** energy_d, void** forces_d, void** virials_d, void** energy_h, void** forces_h,
                                   void** virials_h, const hipStream_t* s, const bool do_forces, const int n_sites,
                                   const int n_sites0) {
  hip_check_error(hipDeviceSynchronize());
