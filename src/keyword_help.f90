...
!     ---- Experimental data (MAD)
      ...
      if (every .or. .not. gap_only) then
         write (*, '(A)') '  ir_auxiliary_restart_file            [string, default none]'
         write (*, '(A)') '      The restart file which specifies the auxiliary coordinates, Format:'
         write (*, '(A)') '      n_freq X11 X12 Z13 P11 P12 P13 eff_mass omega R_exp g_k'
         write (*, '(A)') '      -> see ir_auxiliary_variable, t_beg_aux, t_end_aux'
         write (*, '(A)') ''
      end if
      if (every .or. .not. gap_only) then
         write (*, '(A)') '  ir_auxiliary_variable                [logical, default false]'
         write (*, '(A)') '      This turns on the auxiliary variable in temporal MAD, which'
         write (*, '(A)') '      formulates MAD in another way: using a bath of oscillators which'
         write (*, '(A)') '      have fictitious positions and momenta, where each oscillator is'
         write (*, '(A)') '      attributed to a given frequency (wavenumber) to match from the'
         write (*, '(A)') '      spectrum. These are coupled to the physical system by g_k coupling'
         write (*, '(A)') '      terms which are tuned by according to the magnitude of the'
         write (*, '(A)') '      experiment and the magnitude of the dipole autocorrelation function'
         write (*, '(A)') '      that turbogap gives. The magnitudes of these oscillators give the'
         write (*, '(A)') '      ir_prediction which is compared to the experimental data, and this'
         write (*, '(A)') '      is optimized during the run.'
         write (*, '(A)') '      -> see md'
         write (*, '(A)') ''
      end if
      if (every .or. (mode == 'md')) then
         write (*, '(A)') '  ir_bias_mode                         [string, default acf]'
         write (*, '(A)') '      Which IR bias to run: "acf" (the default) or "xl". Under "acf" the'
         write (*, '(A)') '      spectrum is the cosine transform of an autocorrelation of the stored'
         write (*, '(A)') '      dipoles and the MAD force is the gradient of the mismatch with'
         write (*, '(A)') '      respect to the newest configuration. Under "xl" it is the'
         write (*, '(A)') '      auxiliary-variable bias: a bank of damped resonators, a quadrature'
         write (*, '(A)') '      pair per fitted frequency, integrated alongside the atoms and driven'
         write (*, '(A)') '      by the dipole. The bank IS the Fourier transform -- a Lorentzian'
         write (*, '(A)') '      filter bank written as equations of motion -- so no trajectory is'
         write (*, '(A)') '      stored, there is no hard window for a frame to fall out of, and the'
         write (*, '(A)') '      resolution is set by the damping rather than by a longest lag. The'
         write (*, '(A)') '      bias is the exact gradient of the spectral mismatch with respect to'
         write (*, '(A)') '      the dipole that drives the bank. The price is one stored frame of'
         write (*, '(A)') '      lag: the gradient cannot be formed until the descriptor pass that'
         write (*, '(A)') '      produced the dipole is over, so it is contracted against the next'
         write (*, '(A)') '      frame''s dipole gradient. That is what lets it be contracted INSIDE'
         write (*, '(A)') '      the pass, with no (3,3,n_atoms) tensor formed and a third of the'
         write (*, '(A)') '      work in the descriptor term. The two modes share the experimental'
         write (*, '(A)') '      grid, ir_nu_min, ir_nu_max, ir_nu_power, ir_match_scale,'
         write (*, '(A)') '      ir_match_offset, ir_weight_by_spacing, ir_stride and'
      ...
      if (every .or. .not. gap_only) then
         write (*, '(A)') '  ir_damping                           [real, default -1.0_dp, cm^-1]'
         write (*, '(A)') '      The damping of the oscillators, it is equivalently the'
         write (*, '(A)') '      bandwidth/resolution The default 10 cm^-1'
         write (*, '(A)') '      -> see ir_auxiliary_variable, ir_eff_mass, t_beg_aux'
         write (*, '(A)') ''
      end if
      if (every .or. .not. gap_only) then
         write (*, '(A)') '  ir_eff_mass                          [real, default -1.0_dp, amu]'
         write (*, '(A)') '      The effective mass for the oscillators in ir_auxiliary_variable The'
         write (*, '(A)') '      default is 100amu'
         write (*, '(A)') '      -> see ir_auxiliary_variable, ir_damping, t_beg_aux'
         write (*, '(A)') ''
      end if
      if (every .or. (mode == 'md')) then
      ...
      if (every .or. .not. gap_only) then
         write (*, '(A)') '  t_beg_aux                            [real, default 300.0_dp, K]'
         write (*, '(A)') '      The auxiliary variable(s) initial temperature, by default this is'
         write (*, '(A)') '      t_beg if not specified. This can be used for ir_auxiliary_variable'
         write (*, '(A)') '      or d-AFED (not implemented yet).'
         write (*, '(A)') '      -> see ir_auxiliary_variable, ir_damping, ir_eff_mass'
         write (*, '(A)') ''
      end if
      if (every .or. .not. gap_only) then
         write (*, '(A)') '  t_end_aux                            [real, default 300.0_dp, K]'
         write (*, '(A)') '      The auxiliary variable(s) initial temperature, by default this is'
         write (*, '(A)') '      t_end if not specified. This can be used for ir_auxiliary_variable'
         write (*, '(A)') '      or d-AFED (not implemented yet).'
         write (*, '(A)') '      -> see ir_auxiliary_variable, ir_damping, ir_eff_mass'
         write (*, '(A)') ''
      end if
