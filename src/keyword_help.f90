...
!     ---- General
      ...
      if (every .or. (mode == 'md')) then
         write (*, '(A)') '  do_ir                                [logical, default false]'
         write (*, '(A)') '      Predict an IR spectrum from the trajectory. The total dipole is'
         write (*, '(A)') '      accumulated over the whole run and transformed into ir_spectrum.dat,'
         write (*, '(A)') '      whose header records the sampling interval, the Nyquist limit, the'
         write (*, '(A)') '      resolution and hence the band over which the result can be read.'
         write (*, '(A)') '      Needs a dipole model in the potential file, but no experimental'
         write (*, '(A)') '      spectrum and no exp_* keywords: nothing is fitted and no force is'
         write (*, '(A)') '      added. Implies write_ir, so ir_prediction.dat carries the same'
         write (*, '(A)') '      spectrum over the whole trajectory. The ensemble is the entire run'
         write (*, '(A)') '      rather than a rolling window, so the resolution follows from'
         write (*, '(A)') '      md_nsteps unless ir_resolution asks for one the run is long enough'
         write (*, '(A)') '      to give. Naming "ir" in exp_labels instead is the other thing --'
         write (*, '(A)') '      that biases the trajectory towards an experiment.'
         write (*, '(A)') '      -> see ir_stride, ir_resolution, ir_nu_min, ir_nu_max,'
         write (*, '(A)') '         ir_lag_factor, ir_n_samples, exp_labels'
         write (*, '(A)') ''
      end if
      if (every .or. .not. gap_only) then
      ...
      if (every .or. (mode == 'md')) then
         write (*, '(A)') '  ir_aux_damping                       [real, default 10.0, cm^-1]'
         write (*, '(A)') '      Bandwidth Gamma_k of each resonator under ir_bias_mode = aux: the'
         write (*, '(A)') '      width of one channel of the filter bank, which is what the'
         write (*, '(A)') '      experimental resolution actually is. It appears in the'
         write (*, '(A)') '      linear-response calibration of the couplings, g_k = mu_k w_k'
         write (*, '(A)') '      sqrt(Gamma_k <R_target^2>/S_MM(w_k)), so it sets how hard the dipole'
         write (*, '(A)') '      has to push to reach a given amplitude. 5 to 10 cm^-1 is the usual'
         write (*, '(A)') '      experimental figure.'
         write (*, '(A)') '      -> needs ir_bias_mode; see ir_bias_mode, ir_aux_eff_mass,'
         write (*, '(A)') '         ir_resolution'
         write (*, '(A)') ''
      end if
      if (every .or. (mode == 'md')) then
         write (*, '(A)') '  ir_aux_eff_mass                      [real, default 100.0, amu]'
         write (*, '(A)') '      Fictitious mass of each resonator in the envelope-targeted bank,'
         write (*, '(A)') '      ir_bias_mode = aux. Choose it well above an atomic mass (10^2 to'
         write (*, '(A)') '      10^3 amu) so the auxiliary variables evolve more slowly than the'
         write (*, '(A)') '      nuclei and never set the timestep. It largely cancels out of the'
         write (*, '(A)') '      amplitude the bank reports, because the coupling is calibrated'
         write (*, '(A)') '      against it.'
         write (*, '(A)') '      -> needs ir_bias_mode; see ir_bias_mode, ir_aux_damping,'
         write (*, '(A)') '         ir_aux_tau'
         write (*, '(A)') ''
      end if
      if (every .or. (mode == 'md')) then
         write (*, '(A)') '  ir_aux_eta_max                       [real, default 0.1]'
         write (*, '(A)') '      Anti-windup clamp on the feedback friction under ir_bias_mode = aux,'
         write (*, '(A)') '      as a fraction of each resonator''s own frequency. When a target is'
         write (*, '(A)') '      unreachable the integral in etadot winds up without bound, and past'
         write (*, '(A)') '      |eta| = 2 w_k the channel is overdamped: it stops resonating and'
         write (*, '(A)') '      would report an intensity for a frequency it can no longer sense.'
         write (*, '(A)') '      The default of 0.1 leaves the resonance essentially untouched, since'
         write (*, '(A)') '      the frequency shift it permits is second order, sqrt(w^2 -'
         write (*, '(A)') '      (eta/2)^2).'
         write (*, '(A)') '      -> needs ir_bias_mode; see ir_bias_mode, ir_aux_tau, ir_aux_gain'
         write (*, '(A)') ''
      end if
      if (every .or. (mode == 'md')) then
         write (*, '(A)') '  ir_aux_gain                          [real, default -1.0, 1/fs]'
         write (*, '(A)') '      Proportional gain kappa_k of the amplitude controller under'
         write (*, '(A)') '      ir_bias_mode = aux, as a rate. Negative, the default, means'
         write (*, '(A)') '      2/ir_aux_tau, which critically damps the control loop: the'
         write (*, '(A)') '      linearised envelope obeys nu'''' + kappa nu'' + nu/tau^2 = 0. Setting'
         write (*, '(A)') '      it to zero leaves a pure integral controller, which is MARGINALLY'
         write (*, '(A)') '      STABLE and rings forever with period 2 pi ir_aux_tau -- and the'
         write (*, '(A)') '      ringing is in the reported spectrum, because the resonator amplitude'
         write (*, '(A)') '      is the observable.'
         write (*, '(A)') '      -> needs ir_bias_mode; see ir_bias_mode, ir_aux_tau,'
         write (*, '(A)') '         ir_aux_eta_max'
         write (*, '(A)') ''
      end if
      if (every .or. (mode == 'md')) then
         write (*, '(A)') '  ir_aux_restart_file                  [string, default ir_aux_restart.dat]'
         write (*, '(A)') '      Where the envelope-targeted bank is persisted under ir_bias_mode ='
         write (*, '(A)') '      aux, written on the trajectory''s own schedule so the bank on disk is'
         write (*, '(A)') '      never newer than the positions it belongs to. It carries X, P and'
         write (*, '(A)') '      eta per mode along with the calibrated couplings, which are state in'
         write (*, '(A)') '      the same sense the velocities are. "none" disables it. The loader'
         write (*, '(A)') '      refuses a file whose frequency grid disagrees with the current run'
         write (*, '(A)') '      rather than adopt it, and a refused or missing file is not fatal:'
         write (*, '(A)') '      the run calibrates a fresh bank.'
         write (*, '(A)') '      -> needs ir_bias_mode; see ir_bias_mode, ir_restart_file,'
         write (*, '(A)') '         ir_xl_restart_file'
         write (*, '(A)') ''
      end if
      if (every .or. (mode == 'md')) then
         write (*, '(A)') '  ir_aux_tau                           [real, default 500.0, fs]'
         write (*, '(A)') '      Relaxation time of the amplitude controller under ir_bias_mode ='
         write (*, '(A)') '      aux. The friction eta_k integrates the envelope error as etadot ='
         write (*, '(A)') '      (R^2/R_target^2 - 1)/tau^2, so tau is how quickly the bank is pulled'
         write (*, '(A)') '      back onto the experimental amplitude. It must be long compared with'
         write (*, '(A)') '      1/w_k: the controller acts on the cycle-averaged envelope, and that'
         write (*, '(A)') '      averaging is what makes the default gain critical.'
         write (*, '(A)') '      -> needs ir_bias_mode; see ir_bias_mode, ir_aux_gain,'
         write (*, '(A)') '         ir_aux_eta_max'
         write (*, '(A)') ''
      end if
      if (every .or. (mode == 'md')) then
         write (*, '(A)') '  ir_bias_mode                         [string, default acf]'
         write (*, '(A)') '      Which IR machinery to run: "acf" (the default), "xl" or "fft". It'
         write (*, '(A)') '      selects the estimator for prediction as well as for the bias,'
         write (*, '(A)') '      despite the name. Under "fft" the spectrum comes from ir_fft.f90, a'
         write (*, '(A)') '      translation of TNEP/spectroscopy.py: the correlation is computed'
         write (*, '(A)') '      over the whole stored ensemble by FFT, the longest lag is'
         write (*, '(A)') '      ir_fft_acf_ratio of it, the quantum correction is explicit and'
         write (*, '(A)') '      defaults to the harmonic w(1 - exp(-hbar w/kT)) rather than a fixed'
         write (*, '(A)') '      nu^2, and the result is smoothed. The MAD force is the exact'
         write (*, '(A)') '      gradient of the mismatch with respect to the newest dipole, as under'
         write (*, '(A)') '      "acf", and goes through the same dmu/dr contraction. It is also the'
         write (*, '(A)') '      only mode available to `turbogap predict`, where a trajectory on'
         write (*, '(A)') '      disk is read frame by frame and the interval comes from the time='
         write (*, '(A)') '      tags. Under "acf" the spectrum is the cosine transform of an'
         write (*, '(A)') '      autocorrelation of the stored dipoles and the MAD force is the'
         write (*, '(A)') '      gradient of the mismatch with respect to the newest configuration.'
         write (*, '(A)') '      Under "xl" it is the auxiliary-variable bias: a bank of damped'
         write (*, '(A)') '      resonators, a quadrature pair per fitted frequency, integrated'
         write (*, '(A)') '      alongside the atoms and driven by the dipole. The bank IS the'
         write (*, '(A)') '      Fourier transform -- a Lorentzian filter bank written as equations'
         write (*, '(A)') '      of motion -- so no trajectory is stored, there is no hard window for'
         write (*, '(A)') '      a frame to fall out of, and the resolution is set by the damping'
         write (*, '(A)') '      rather than by a longest lag. The bias is the exact gradient of the'
         write (*, '(A)') '      spectral mismatch with respect to the dipole that drives the bank.'
         write (*, '(A)') '      The price is one stored frame of lag: the gradient cannot be formed'
         write (*, '(A)') '      until the descriptor pass that produced the dipole is over, so it is'
         write (*, '(A)') '      contracted against the next frame''s dipole gradient. That is what'
         write (*, '(A)') '      lets it be contracted INSIDE the pass, with no (3,3,n_atoms) tensor'
         write (*, '(A)') '      formed and a third of the work in the descriptor term. The two modes'
         write (*, '(A)') '      share the experimental grid, ir_nu_min, ir_nu_max, ir_nu_power,'
         write (*, '(A)') '      ir_match_scale, ir_match_offset, ir_weight_by_spacing, ir_stride and'
         write (*, '(A)') '      exp_energy_scales. Everything from ir_lag_factor to ir_tau_mem'
         write (*, '(A)') '      describes an autocorrelation and is ignored under "xl".'
         write (*, '(A)') '      -> needs exp_labels; see ir_xl_tau_mem, ir_xl_n_modes,'
         write (*, '(A)') '         ir_xl_amplitude, ir_xl_warm_factor, ir_acf_mode'
         write (*, '(A)') ''
      end if
      if (every .or. (mode == 'md')) then
         ...
         write (*, '(A)') '      averaged, N - tau. The unbiased estimator is unbiased and is also'
         write (*, '(A)') '      not a positive semi-definite sequence, so its cosine transform can'
         write (*, '(A)') '      be negative -- and an absorption spectrum cannot be. The biased one'
         write (*, '(A)') '      IS positive semi-definite: its transform is a periodogram (Percival'
         write (*, '(A)') '      & Walden ch. 6; Numerical Recipes 13.4), at the price of scaling'
         write (*, '(A)') '      C(tau) by (1 - tau/N). Since the lag window already tapers far'
         write (*, '(A)') '      harder than that, nothing is lost. Note that guarantee covers the'
         write (*, '(A)') '      UNWINDOWED estimate: windowing convolves the spectrum with the'
         write (*, '(A)') '      window kernel, so a kernel that dips negative can still drive the'
         write (*, '(A)') '      result negative. Measured on one trajectory with every other fix on,'
         write (*, '(A)') '      ir_window = "none" gave 124 negative points of 400 while hann, lorch'
         write (*, '(A)') '      and bartlett gave zero. Combine with ir_window = "bartlett", whose'
         write (*, '(A)') '      Fejer kernel is non-negative everywhere, for an unconditional'
         write (*, '(A)') '      guarantee.'
         write (*, '(A)') '      -> see ir_window, ir_subtract_mean, ir_lag_factor'
         write (*, '(A)') ''
      end if
      if (every .or. (mode == 'md' .or. mode == 'predict')) then
         write (*, '(A)') '  ir_fft_acf_ratio                     [real, default 0.1]'
         write (*, '(A)') '      Fraction of the stored ensemble kept as lags under ir_bias_mode ='
         write (*, '(A)') '      fft, GPUMD''s convention (default 0.1). This and nothing else sets'
         write (*, '(A)') '      the frequency resolution: 33356.40952/(ir_fft_acf_ratio * n_frames *'
         write (*, '(A)') '      dt) cm^-1, where dt is the interval between stored frames. Raising'
         write (*, '(A)') '      it buys resolution and pays in variance, because C(tau) at lag tau'
         write (*, '(A)') '      averages over n_frames - tau products and the longest lag kept is'
         write (*, '(A)') '      the least well determined. Set it to 1/ir_lag_factor to resolve'
         write (*, '(A)') '      exactly what the block estimator resolves, which is what to do when'
         write (*, '(A)') '      the point is to compare the two.'
         write (*, '(A)') '      -> needs ir_bias_mode; see ir_bias_mode, ir_lag_factor,'
         write (*, '(A)') '         ir_resolution, ir_fft_smooth_k'
         write (*, '(A)') ''
      end if
      if (every .or. (mode == 'md' .or. mode == 'predict')) then
         write (*, '(A)') '  ir_fft_power_dc_cutoff               [real, default 100.0, cm^-1]'
         write (*, '(A)') '      Wavenumber below which bins are excluded from the POWER spectrum''s'
         write (*, '(A)') '      peak normaliser (default 100 cm^-1). The unweighted power spectrum'
         write (*, '(A)') '      M(w) has a large peak at w = 0 that is diffusion and drift rather'
         write (*, '(A)') '      than a vibration; normalising by it crushes every vibrational'
         write (*, '(A)') '      feature to a per cent of full scale and makes the plot look empty. 0'
         write (*, '(A)') '      keeps the DC bin. It affects only the normalisation of the power'
         write (*, '(A)') '      column, never the intensity and never the bias.'
         write (*, '(A)') '      -> needs ir_bias_mode; see ir_bias_mode'
         write (*, '(A)') ''
      end if
      if (every .or. (mode == 'md' .or. mode == 'predict')) then
         write (*, '(A)') '  ir_fft_quantum_correction            [string, default harmonic]'
         write (*, '(A)') '      Which quantum correction factor turns the classical lineshape into'
         write (*, '(A)') '      an absorption, under ir_bias_mode = fft. "harmonic" (the default) is'
         write (*, '(A)') '      w(1 - exp(-hbar w/kT)), which is what GPUMD and Xu et al. apply.'
         write (*, '(A)') '      "classical" (alias "quadratic") is w^2, which is the same correction'
         write (*, '(A)') '      written the other way round and is what mad_ir.f90 applies with'
         write (*, '(A)') '      ir_nu_power = 2. "linear" is w and "none" is 1. The first two agree'
         write (*, '(A)') '      only for hbar w << kT: at 300 K that is below about 208 cm^-1, and'
         write (*, '(A)') '      by 3400 cm^-1 they differ by a factor of 16 in the weight given to'
         write (*, '(A)') '      the band. No fitted scale can absorb that, because it is a tilt and'
         write (*, '(A)') '      not a factor. When two spectra of the same trajectory disagree about'
         write (*, '(A)') '      band heights, check this before anything else.'
         write (*, '(A)') '      -> needs ir_bias_mode; see ir_nu_power, ir_fft_temperature,'
         write (*, '(A)') '         ir_bias_mode'
         write (*, '(A)') ''
      end if
      if (every .or. (mode == 'md' .or. mode == 'predict')) then
         write (*, '(A)') '  ir_fft_smooth_k                      [integer, default 10]'
         write (*, '(A)') '      Smoothing width under ir_bias_mode = fft, in BINS rather than cm^-1'
         write (*, '(A)') '      (default 10). For ir_fft_smooth_kind = gaussian it is the kernel'
         write (*, '(A)') '      FWHM; for box the moving-average width. 0 or 1 disables it. One bin'
         write (*, '(A)') '      is 33356.40952/((2*n_lag - 1)*dt) cm^-1, which is half the'
         write (*, '(A)') '      resolution, so the default broadens a band by about five resolution'
         write (*, '(A)') '      elements: a feature narrower than ir_fft_smooth_k bins is the'
         write (*, '(A)') '      smoother''s shape and not the sample''s. Under a MAD bias the'
         write (*, '(A)') '      smoothing is differentiated exactly along with everything else, so'
         write (*, '(A)') '      it changes what is being fitted rather than only how it looks.'
         write (*, '(A)') '      -> needs ir_bias_mode; see ir_fft_smooth_kind, ir_fft_acf_ratio,'
         write (*, '(A)') '         ir_bias_mode'
         write (*, '(A)') ''
      end if
      if (every .or. (mode == 'md' .or. mode == 'predict')) then
         write (*, '(A)') '  ir_fft_smooth_kind                   [string, default gaussian]'
         write (*, '(A)') '      "gaussian" (default) or "box". Gaussian is a truncated Gaussian'
         write (*, '(A)') '      kernel with clamped edges, matching'
         write (*, '(A)') '      scipy.ndimage.gaussian_filter1d(mode="nearest"); it has no'
         write (*, '(A)') '      sidelobes. Box is the moving average GPUMD uses, in numpy''s'
         write (*, '(A)') '      mode="valid": it has sinc sidelobes and it SHORTENS the spectrum by'
         write (*, '(A)') '      ir_fft_smooth_k - 1 bins, shifting the first wavenumber up by half a'
         write (*, '(A)') '      width. Both are exact under differentiation.'
         write (*, '(A)') '      -> needs ir_bias_mode; see ir_fft_smooth_k, ir_bias_mode'
         write (*, '(A)') ''
      end if
      if (every .or. (mode == 'md' .or. mode == 'predict')) then
         write (*, '(A)') '  ir_fft_temperature                   [real, default -1.0, K]'
         write (*, '(A)') '      Temperature in K for the harmonic quantum correction. A value of'
         write (*, '(A)') '      zero or less, which is the default, means take t_beg: the run'
         write (*, '(A)') '      already knows its target temperature and making the user repeat it'
         write (*, '(A)') '      only creates a second number that can disagree with the first. Set'
         write (*, '(A)') '      it explicitly when post-processing a trajectory whose temperature is'
         write (*, '(A)') '      not the t_beg of the current input.'
         write (*, '(A)') '      -> needs ir_fft_quantum_correction; see'
         write (*, '(A)') '         ir_fft_quantum_correction, t_beg'
         write (*, '(A)') ''
      end if
      if (every .or. (mode == 'md' .or. mode == 'predict')) then
         write (*, '(A)') '  ir_fft_write_dipoles                 [logical, default true]'
         write (*, '(A)') '      Also write ir_fft_dipoles.dat: the time and total dipole of every'
         write (*, '(A)') '      frame the spectrum was computed from (default .true.). Four columns'
         write (*, '(A)') '      of text, and it makes the result reproducible outside TurboGAP --'
         write (*, '(A)') '      feeding columns 2 to 4 to TNEP/spectroscopy.py''s compute_ir_spectrum'
         write (*, '(A)') '      reproduces ir_fft_spectrum.dat, which is how the two implementations'
         write (*, '(A)') '      are checked against each other.'
         write (*, '(A)') '      -> needs ir_bias_mode; see ir_bias_mode, do_ir'
         write (*, '(A)') ''
      end if
      if (every .or. (mode == 'predict')) then
         write (*, '(A)') '  ir_frame_dt                          [real, default -1.0, fs]'
         write (*, '(A)') '      Interval between frames in fs for `turbogap predict` with do_ir,'
         write (*, '(A)') '      used ONLY when no frame in the trajectory carries a time= tag.'
         write (*, '(A)') '      TurboGAP''s own trajectory_out.xyz always carries one, and then the'
         write (*, '(A)') '      labels are believed and this is ignored. That is deliberate: the'
         write (*, '(A)') '      frame interval is not the MD timestep but write_xyz times it, and'
         write (*, '(A)') '      getting it wrong rescales the entire wavenumber axis by that ratio'
         write (*, '(A)') '      while producing a spectrum that looks perfectly reasonable. A'
         write (*, '(A)') '      trajectory in which SOME frames are labelled is refused outright'
         write (*, '(A)') '      rather than patched up with this.'
         write (*, '(A)') '      -> needs do_ir; see do_ir, ir_frame_dt_tol, ir_bias_mode'
         write (*, '(A)') ''
      end if
      if (every .or. (mode == 'predict')) then
         write (*, '(A)') '  ir_frame_dt_tol                      [real, default 1.e-3]'
         write (*, '(A)') '      How far a consecutive frame spacing may stray from the mean, as a'
         write (*, '(A)') '      fraction, before the trajectory is refused as unevenly sampled'
         write (*, '(A)') '      (default 1e-3). An unevenly sampled series has no Fourier transform'
         write (*, '(A)') '      of the kind being computed, and the failure is otherwise entirely'
         write (*, '(A)') '      silent. The default is loose because time= is written with finite'
         write (*, '(A)') '      precision; it is there to catch a trajectory with frames missing, or'
         write (*, '(A)') '      two runs concatenated, not to police the last digit.'
         write (*, '(A)') '      -> needs do_ir; see ir_frame_dt, do_ir'
         write (*, '(A)') ''
      end if
      if (every .or. (mode == 'md')) then
      ...
      if (every .or. (mode == 'md')) then
         write (*, '(A)') '  ir_match_offset                      [logical, default false]'
         write (*, '(A)') '      Fit an additive baseline b alongside the scale s, comparing s*I_calc'
         write (*, '(A)') '      + b with the experiment (default .false.; requires ir_match_scale).'
         write (*, '(A)') '      Needed when the experimental file has had a baseline removed such'
         write (*, '(A)') '      that its minimum is exactly zero, as tests/mad_ir''s process_data.py'
         write (*, '(A)') '      does: the model spectrum is strictly positive, so without an offset'
         write (*, '(A)') '      the residual in the transparency window can never vanish, and a'
         write (*, '(A)') '      quadratic loss responds by shrinking the whole prediction --'
         write (*, '(A)') '      fighting the very band intensity the bias is trying to build. Both s'
         write (*, '(A)') '      and b sit at their own least-squares optimum, so the gradient picks'
         write (*, '(A)') '      up no extra term from either. CAUTION: the two-parameter solve is'
         write (*, '(A)') '      unconstrained and can return a NEGATIVE scale when the predicted'
         write (*, '(A)') '      shape does not resemble the experiment -- on this potential, whose'
         write (*, '(A)') '      dipole model produces no vibrational bands, it returned s = -9.7e-9'
         write (*, '(A)') '      with b = +2.70. Since dL/dI carries a factor of s, that would'
         write (*, '(A)') '      reverse the bias and drive the model away from the bands it is being'
         write (*, '(A)') '      asked to grow. The code detects s <= 0 and falls back to the'
         write (*, '(A)') '      scale-only fit with b = 0; the header reports the offset actually'
         write (*, '(A)') '      used, so a run that took the fallback is identifiable. If your fits'
         write (*, '(A)') '      keep falling back, the prediction and the experiment disagree in'
         write (*, '(A)') '      shape, not just in baseline.'
         write (*, '(A)') '      -> see ir_match_scale, exp_energy_scales, exp_data_files'
         write (*, '(A)') ''
      end if
      if (every .or. (mode == 'md')) then
      ...
      if (every .or. (mode == 'md')) then
         write (*, '(A)') '  ir_n_samples                         [integer, default 0]'
         write (*, '(A)') '      Number of points on the predicted spectrum''s grid. The default,'
         write (*, '(A)') '      zero, puts one point per resolution element, which is all the'
         write (*, '(A)') '      transform can carry; a larger number draws the same information as a'
         write (*, '(A)') '      smoother curve. Prediction runs only -- a fit uses the experimental'
         write (*, '(A)') '      grid.'
         write (*, '(A)') '      -> see do_ir, ir_resolution'
         write (*, '(A)') ''
      end if
      if (every .or. (mode == 'md')) then
      ...
      if (every .or. (mode == 'md')) then
         ...
         write (*, '(A)') '      -> sets ir_resolution_set; see exp_labels, exp_energy_scales,'
         write (*, '(A)') '         ir_lag_factor, ir_stride'
         write (*, '(A)') ''
      end if
      if (every .or. (mode == 'md')) then
      ...
      if (every .or. (mode == 'md')) then
         write (*, '(A)') '  ir_subtract_mean                     [logical, default true]'
         write (*, '(A)') '      Correlate the dipole FLUCTUATION mu - <mu> rather than mu itself'
         write (*, '(A)') '      (default .true.). Linear response gives the spectrum as the'
         write (*, '(A)') '      transform of <dM(0).dM(t)> with dM = M - <M>; a static dipole does'
         write (*, '(A)') '      not absorb, and the mean drops out of the derivation for that'
         write (*, '(A)') '      reason. The usual defence for skipping it -- "a constant only puts a'
         write (*, '(A)') '      delta at nu = 0, which the nu^2 prefactor kills" -- is false,'
         write (*, '(A)') '      because only the |<mu>|^2 piece of the un-centred correlation is'
         write (*, '(A)') '      constant. The cross term is a partial sum of a mean-zero series'
         write (*, '(A)') '      divided by its own length, i.e. a random walk in tau whose amplitude'
         write (*, '(A)') '      GROWS with lag. On a 100-molecule water buffer at 2 fs, |<mu>|^2 was'
         write (*, '(A)') '      86% of C(0) and past ~300 fs of lag the cross term was 17x the real'
         write (*, '(A)') '      correlation, shifting the spectrum by 19% at the O-H stretch. Set'
         write (*, '(A)') '      .false. only to reproduce results from before this was fixed.'
         write (*, '(A)') '      -> see ir_estimator, ir_taper_partial, exp_labels'
         write (*, '(A)') ''
      end if
      if (every .or. (mode == 'md')) then
         write (*, '(A)') '  ir_taper_partial                     [logical, default true]'
         write (*, '(A)') '      Rebuild the lag window over the lags actually available while the'
         write (*, '(A)') '      ensemble is still filling (default .true.). Only affects do_ir'
         write (*, '(A)') '      prediction runs, where the buffer fills over the whole trajectory.'
         write (*, '(A)') '      The window is sized for n_lag, but early on only n_stored - 1 lags'
         write (*, '(A)') '      exist, so with this off the autocorrelation is truncated at a point'
         write (*, '(A)') '      where the window is still near 1 -- a boxcar cut, which convolves'
         write (*, '(A)') '      the spectrum with a sinc and makes it ring. How much it matters'
         write (*, '(A)') '      depends on ir_estimator: dividing C(tau) by N rather than N - tau'
         write (*, '(A)') '      multiplies it by (1 - tau/N), which is itself a triangular taper, so'
         write (*, '(A)') '      the biased estimator applies an implicit Bartlett window that has'
         write (*, '(A)') '      already fallen to 1/N at the truncation point. Measured with the'
         write (*, '(A)') '      window sized for n_lag = 417 and only 51 frames stored, ir_estimator'
         write (*, '(A)') '      = unbiased gave 160 negative points of 400 and ir_estimator = biased'
         write (*, '(A)') '      gave 0. With the default biased estimator this switch therefore does'
         write (*, '(A)') '      not change the ringing; what it still buys is a header that quotes'
      ...
      if (every .or. (mode == 'md')) then
         write (*, '(A)') '  ir_weight_by_spacing                 [logical, default true]'
         write (*, '(A)') '      Weight each experimental point by the local grid spacing (default'
         write (*, '(A)') '      .true.), so that the loss is an integral over frequency rather than'
         write (*, '(A)') '      a sum over however the experimental file happened to be sampled. The'
         write (*, '(A)') '      fit grid is the experimental grid -- interpolating the experiment'
         write (*, '(A)') '      would invent structure between its points and then fit to it -- but'
         write (*, '(A)') '      experimental grids are rarely uniform. The Downing & Williams water'
         write (*, '(A)') '      data shipped with tests/mad_ir is sampled at 23.9 cm^-1 across the'
         write (*, '(A)') '      O-H stretch and ~55 cm^-1 elsewhere, so 45% of an unweighted loss'
         write (*, '(A)') '      falls in the top quarter of the range: a weighting chosen by whoever'
         write (*, '(A)') '      digitised the paper, not by the physics. Weights are normalised to'
         write (*, '(A)') '      mean 1, so exp_energy_scales keeps its magnitude.'
         write (*, '(A)') '      -> see exp_energy_scales, ir_nu_min, ir_nu_max, exp_data_files'
         write (*, '(A)') ''
      end if
      if (every .or. (mode == 'md')) then
         write (*, '(A)') '  ir_window                            [string, default hann]'
         write (*, '(A)') '      Lag window applied to the autocorrelation before the cosine'
         write (*, '(A)') '      transform: "hann" (default), "bartlett", "lorch", "welch" or "none".'
         write (*, '(A)') '      This is not optional in substance -- "none" is a boxcar whose kernel'
         write (*, '(A)') '      is a sinc, with side lobes at -13.3 dB and ringing to -21.7%, which'
         write (*, '(A)') '      drives an absorption spectrum negative. The choice is which taper,'
         write (*, '(A)') '      not whether. Hann has the lowest far-field leakage of the set (13 dB'
         write (*, '(A)') '      below the next best, because its kernel falls as 1/f^3) and is the'
         write (*, '(A)') '      only one whose main lobe FWHM equals the resolution ir_resolution'
         write (*, '(A)') '      advertises. Bartlett is the one shape whose kernel (Fejer) is'
         write (*, '(A)') '      non-negative everywhere, so with ir_estimator = "biased" it makes a'
         write (*, '(A)') '      non-negative spectrum a theorem rather than an observation, at the'
         write (*, '(A)') '      cost of 5 dB more leakage. Lorch is sinc(pi tau / n_lag), the same'
         write (*, '(A)') '      modification function exp_utils applies to g(r) under'
         write (*, '(A)') '      structure_factor_window, offered so the IR path can make the'
         write (*, '(A)') '      approximation the XRD path makes; it buys 13% narrower bands for 5'
         write (*, '(A)') '      dB more leakage and 1.8x more ringing. "none" is for demonstrating'
         write (*, '(A)') '      the ringing, not for production. See ana/formalism_windows.py for'
         write (*, '(A)') '      the measured kernels.'
         write (*, '(A)') '      -> see exp_labels, exp_energy_scales, ir_resolution,'
         write (*, '(A)') '         ir_estimator, structure_factor_window'
         write (*, '(A)') ''
      end if
      if (every .or. (mode == 'md')) then
         write (*, '(A)') '  ir_write_spectrum                    [logical, default false]'
         write (*, '(A)') '      The old name for write_ir, kept because inputs use it. Sets'
         write (*, '(A)') '      write_ir.'
         write (*, '(A)') '      -> sets write_ir; see write_ir, exp_labels, exp_energy_scales'
         write (*, '(A)') ''
      end if
      if (every .or. (mode == 'md')) then
         write (*, '(A)') '  ir_xl_amplitude                      [string, default coherent]'
         write (*, '(A)') '      How the collective amplitude R_k that the experiment is compared'
         write (*, '(A)') '      with is built from the bank: "coherent" (the default) sums the'
         write (*, '(A)') '      resonators first and squares after, |sum_i x|^2 + |sum_i y|^2, and'
         write (*, '(A)') '      "incoherent" squares first and sums after. Coherent is the actual'
         write (*, '(A)') '      infrared observable -- absorption comes from the correlation of the'
         write (*, '(A)') '      TOTAL dipole, and the cross terms between sites are'
         write (*, '(A)') '      transition-dipole coupling, not noise. Incoherent throws those away,'
         write (*, '(A)') '      and exists because a per-site target is only meaningful against'
         write (*, '(A)') '      per-site amplitudes. Coherent also has no spring-constant shift and'
         write (*, '(A)') '      so nothing to clamp; under "incoherent" the bias detunes each'
         write (*, '(A)') '      resonator, and a shift large enough to inflect the spring is clamped'
         write (*, '(A)') '      and counted.'
         write (*, '(A)') '      -> needs ir_bias_mode; see ir_bias_mode, ir_xl_n_modes'
         write (*, '(A)') ''
      end if
      if (every .or. (mode == 'md')) then
         write (*, '(A)') '  ir_xl_max_memory                     [real, default 4096.0, MB]'
         write (*, '(A)') '      Refuse to start if the resonator bank would need more than this many'
         write (*, '(A)') '      MB on one rank. The bank is replicated per rank, so this is per rank'
         write (*, '(A)') '      and not per node. Zero disables the check.'
         write (*, '(A)') '      -> needs ir_bias_mode; see ir_bias_mode, ir_xl_n_modes'
         write (*, '(A)') ''
      end if
      if (every .or. (mode == 'md')) then
         write (*, '(A)') '  ir_xl_n_modes                        [integer, default 64]'
         write (*, '(A)') '      How many fitted frequencies get resonators. Under ir_xl_amplitude ='
         write (*, '(A)') '      "coherent" the bank costs 96 * ir_xl_n_modes bytes and this hardly'
         write (*, '(A)') '      matters; under "incoherent" it costs that times the number of atoms,'
         write (*, '(A)') '      on every rank, and this is the memory knob. The frequencies chosen'
         write (*, '(A)') '      are a SUBSET of the experimental grid, evenly spaced in index and'
         write (*, '(A)') '      endpoints included -- never a resampling, because interpolating the'
         write (*, '(A)') '      experiment invents structure between its points and then fits to it.'
         write (*, '(A)') '      Asking for more than (ir_nu_max - ir_nu_min) / d(nu) modes buys'
         write (*, '(A)') '      nothing: the bank cannot resolve bins narrower than its own'
         write (*, '(A)') '      bandwidth. A value at or above the number of fitted points keeps all'
         write (*, '(A)') '      of them.'
         write (*, '(A)') '      -> needs ir_bias_mode; see ir_bias_mode, ir_xl_tau_mem,'
         write (*, '(A)') '         ir_xl_max_memory'
         write (*, '(A)') ''
      end if
      if (every .or. (mode == 'md')) then
         write (*, '(A)') '  ir_xl_restart_file                   [string, default ir_xl_restart.dat]'
         write (*, '(A)') '      Where the resonator bank is written, and read from on a restart.'
         write (*, '(A)') '      "none" turns both off. Separate from ir_restart_file because a bank'
         write (*, '(A)') '      is not a history buffer: a file written under a different'
         write (*, '(A)') '      ir_xl_tau_mem, ir_xl_g, ir_stride, mode grid or ir_xl_amplitude'
         write (*, '(A)') '      describes a different filter and is refused rather than adopted. A'
         write (*, '(A)') '      refusal is not fatal -- the run charges a fresh bank and says so.'
         write (*, '(A)') '      -> needs ir_bias_mode; see ir_bias_mode, ir_restart_file'
         write (*, '(A)') ''
      end if
      if (every .or. (mode == 'md')) then
         write (*, '(A)') '  ir_xl_tau_mem                        [real, default 0.0, fs]'
         write (*, '(A)') '      Memory time of the extended-Lagrangian resonators, and so their'
         write (*, '(A)') '      resolution. A resonator damped at gamma = 2/ir_xl_tau_mem is a'
         write (*, '(A)') '      Lorentzian bandpass of full width gamma, which in wavenumbers is'
         write (*, '(A)') '      d(nu) = 33356.40952/(pi ir_xl_tau_mem) -- the same quantity that'
         write (*, '(A)') '      n_lag*md_step*ir_stride sets for the ACF bias, so 4 cm^-1 needs'
         write (*, '(A)') '      about 2650 fs either way. There is no default because there is no'
         write (*, '(A)') '      default resolution. It must exceed the interval between stored'
         write (*, '(A)') '      frames, and it must be long enough that the lowest fitted wavenumber'
         write (*, '(A)') '      is still underdamped, tau > 33356.40952/(2 pi nu_min).'
         write (*, '(A)') '      -> needs ir_bias_mode; see ir_bias_mode, ir_xl_n_modes,'
         write (*, '(A)') '         ir_resolution, ir_stride'
         write (*, '(A)') ''
      end if
      if (every .or. (mode == 'md')) then
         write (*, '(A)') '  ir_xl_warm_factor                    [real, default 3.0]'
         write (*, '(A)') '      Memory times to charge the resonator bank for before any bias is'
         write (*, '(A)') '      applied. The charge-up envelope, 1 - exp(-t/ir_xl_tau_mem), is the'
         write (*, '(A)') '      same at every frequency because the damping is, so the deficit it'
         write (*, '(A)') '      leaves is a pure overall factor and ir_match_scale absorbs it'
         write (*, '(A)') '      exactly; what this waits out is the ringing left by starting the'
         write (*, '(A)') '      bank at rest, which is at w_k and so is not common across the bank.'
         write (*, '(A)') '      Three memory times leaves that at 5% of its initial amplitude.'
         write (*, '(A)') '      -> needs ir_bias_mode; see ir_bias_mode, ir_xl_tau_mem'
         write (*, '(A)') ''
      end if
      if (every .or. .not. gap_only) then
      ...
      if (every .or. .not. gap_only) then
         ...
         write (*, '(A)') '      optimum on GST. Keep it small: the cell list puts int(w/rcut_max)'
         write (*, '(A)') '      bins along each lattice direction, for w the perpendicular width of'
         write (*, '(A)') '      the cell there, so a buffer that pushes w/rcut_max across an integer'
         write (*, '(A)') '      coarsens the grid a whole step and can cost more than the skipped'
         write (*, '(A)') '      rebuilds save. With a large observable cutoff, check'
         write (*, '(A)') '      int(w/(rcut+buffer)) before raising it. It also does not make a'
         write (*, '(A)') '      rebuild cheaper, only rarer, and how much rarer depends on how fast'
         write (*, '(A)') '      the atoms move: at 1 fs steps on hot carbon the default buys a'
         write (*, '(A)') '      factor of two and no more.'
         write (*, '(A)') ''
      end if
      if (every .or. .not. gap_only) then
...
!     ---- Output
      ...
      if (every .or. (mode == 'md')) then
         write (*, '(A)') '  write_ir                             [logical, default false]'
         write (*, '(A)') '      Write the predicted IR spectrum. ir_spectrum.dat holds the current'
         write (*, '(A)') '      one, with the sampling interval, Nyquist limit, resolution and'
         write (*, '(A)') '      trustworthy band in its header, and with the experiment beside it'
         write (*, '(A)') '      when there is one. On every write_xyz step a two-column block is'
         write (*, '(A)') '      also appended to ir_prediction.dat, giving the prediction over the'
         write (*, '(A)') '      whole trajectory; the blocks are separated by one blank line, the'
         write (*, '(A)') '      same format xrd_prediction.dat uses, so gnuplot reaches block i with'
         write (*, '(A)') '      "every :::i::i" rather than with "index". A fitting run additionally'
         write (*, '(A)') '      writes ir_exp.dat, the experiment restricted to the fitted range.'
         write (*, '(A)') '      Needs either do_ir, which implies this switch, or "ir" in'
         write (*, '(A)') '      exp_labels. ir_write_spectrum is the old name for it.'
         write (*, '(A)') '      -> see do_ir, exp_labels, write_xyz, ir_stride'
         write (*, '(A)') ''
      end if
      if (every .or. .not. gap_only) then
