> bilayer_graphene_ts_0.05.dat
for i in $(seq 3.00 0.05 4.00); do
echo $i
cat <<EOF > input
atoms_file = 'bilayer_graphene_$i.xyz'
...
vdw_mbd_rcut = 7.5        ! Cut-off for atoms to include for local MBD energy (vdw_mbd_rcut >= vdw_scs_rcut + vdw_buffer). Type: REAL
...
vdw_omega_ref = 4.d0
EOF
mpirun -np 4 ../bin/turbogap predict | grep "Total energy" | awk '{print $3}' >> bilayer_graphene_ts_0.05.dat
done
