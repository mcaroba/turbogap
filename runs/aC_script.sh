for d in $(seq 0.0 0.5 17.0); do
g="$( bc <<<"$d + 2" )"
echo $d $g
> aC_energies_$d.dat
> aC_scs_timing_$d.dat
> aC_communication_timing_$d.dat
> aC_mbd_timing_$d.dat
cat <<EOF > input
atoms_file = '2829.xyz'
...
grep "Total energy" output | awk '{print $3}' >> aC_energies_$d.dat
grep "SCS timing" output | awk '{print $3}' >> aC_scs_timing_$d.dat
grep "Communication timing" output | awk '{print $3}' >> aC_communication_timing_$d.dat
grep "MBD timing" output | awk '{print $3}' >> aC_mbd_timing_$d.dat
done
