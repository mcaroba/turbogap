...
# Plot the MC data
ax_mc.set_title("MC Energy per atom and N atoms")
...
# Plot the XPS data
ax_xps.set_title("XPS Prediction compared to experiment")
for i in range(n_xps_pred):
    if i + 1 == n_xps_pred:
        ax_xps.plot(xps_pred_data[i,:,0], xps_pred_data[i,:,1], color=plt.cm.magma( np.log(i+1)/np.log(n_xps_pred+1) ), label="Final XPS prediction")
    elif i  == 0:
        ax_xps.plot(xps_pred_data[i,:,0], xps_pred_data[i,:,1], color=plt.cm.magma( np.log(i+1)/np.log(n_xps_pred+1) ), label="Initial XPS prediction")

    else:
        ax_xps.plot(xps_pred_data[i,:,0], xps_pred_data[i,:,1], color=plt.cm.magma(np.log(i+1)/np.log(n_xps_pred+1)))
