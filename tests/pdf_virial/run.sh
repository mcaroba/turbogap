...
#
#   xrd        get_structure_factor_forces_matrix with do_xrd, the live path
#   xrd-plain  get_structure_factor_forces, the same thing without the dgemm
...
make_targets() {
  { preamble
    printf 'write_pair_distribution = .true.\nwrite_structure_factor = .true.\n'
  } >"$WORK/input"
  (cd "$WORK" && "$BIN" predict) >"$WORK/targets.log" 2>&1 ||
    die "the target-generating run failed; see $WORK/targets.log"
  "$PYTHON" "$here/make_targets.py" \
    "$WORK/pair_distribution_total.dat" "$WORK/pdf_target.dat" || die "make_targets"
  "$PYTHON" "$here/make_targets.py" \
    "$WORK/structure_factor_total.dat" "$WORK/sf_target.dat" || die "make_targets"

  # D(r) is a different observable, not a rescaling of g(r), so it needs its
  # own target. exp_input_type below then tells turbogap the file is already
  # D(r); left unsaid it would rebuild the target from the current density on
  # every step, which makes the target itself move with the cell.
  { preamble
    printf "write_pair_distribution = .true.\npair_distribution_output = 'D(r)'\n"
  } >"$WORK/input"
  (cd "$WORK" && "$BIN" predict) >"$WORK/targets.log" 2>&1 ||
    die "the D(r) target-generating run failed; see $WORK/targets.log"
  "$PYTHON" "$here/make_targets.py" \
    "$WORK/pair_distribution_total.dat" "$WORK/pdf_dr_target.dat" || die "make_targets"
}
...
# The xrd_mad case uses exp_energy_scales = 100. The Lorentz-polarization
# factor multiplies the pattern by up to ~117 over this q range and the
# residual enters the energy squared, so at that scale the virial runs past the
# field width of the xyz writer and comes back as two numbers with no space
# between them. Nothing here depends on the scale: it divides out of every
# comparison.
if run_leg xrd; then
  leg "xrd, matrix forces, Lorentz-polarization on" xrd 1.0 \
    "do_xrd     = .true.
xrd_output = 'q*F(q)'
